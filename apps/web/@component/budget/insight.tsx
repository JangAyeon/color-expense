...
const getPrevMonth = ({ year, month }: { year: string; month: string }) => {
  // month는 1~12 기준이라고 가정
  const date = new Date(Number(year), Number(month) - 1); // JS Date는 0-based month
  date.setMonth(date.getMonth() - 1); // 한 달 전으로 이동

  return {
    year: date.getFullYear().toString(),
    month: (date.getMonth() + 1).toString(), // 다시 1-based로 반환
  };
};
...
const Insight: React.FC<InsightProps> = ({
  ...
  if (!budgetHistory || !expenseCategory) return <>sdfa</>;
  return (
    <motion.div
      key="insights"
      custom={direction}
    ...
    >
      <Card>
        ...
        {budgetHistory.history.length > 0 ? (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-blue-50 rounded-lg p-4">
                ...
                <p className="text-xl font-bold">
                  {(
                    budgetHistory.history.reduce(
                      (acc, item) => acc + item.spent,
                      0
                    ) / budgetHistory.history.length
                  ).toLocaleString()}
                  원
                </p>
              </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <p className="text-sm text-gray-600">
                {budgetHistory.history[0]?.spent &&
                budgetHistory.history[1]?.spent &&
                budgetHistory.history[0]?.spent > budgetHistory.history[1].spent
        ...
        prevExpenseCategory.categories.length > 0 ? (
          <>
            <div className="h-64 mb-4">
              <Radar
                data={{
                  labels: expenseCategory.categories.map((cat) => cat.category),
                  datasets: [
                    {
                      label: "이번 달",
                      data: expenseCategory.categories.map((cat) => cat.amount),
          ...
          <div className="flex flex-col h-full items-center justify-center h-40 text-gray-400">
            <svg
      ...
      <Card className="lg:col-span-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="space-y-2">
              {(expenseCategory || prevExpenseCategory).categories.map(
                (category, index) => (
                  <div
                    key={index}
